...
Furnarchy.register(
	...
	(api) => {
		...
		let modalInterval: number | undefined;
		...
		async function handleRemoteMessage(buffer: ArrayBuffer) {
			...
			for (const device of state.devices) {
				...
				if (msg) {
					...
					if (msg.type === "cmd") {
						api.send(msg.cmd);
						// If speech or emote, touch life-support to reset AFK timer
						if (msg.cmd.startsWith('"') || msg.cmd.startsWith(":")) {
							api.use<{ touch: () => void }>("life-support")?.touch();
						}
		...
		api.onConfigure(() => {
			...
			const updateModal = async () => {
					...
					if (state.pendingPairing) {
						if (!state.pendingPairing.isVerified) {
							const words = state.pendingPairing.sasWords
								.map((w) => `<span class="sas-word">${w}</span>`)
								.join("");
                ...
                <div class="text-center">
                  ...
                  <button class="btn-danger btn-full" onclick="window.postMessage({type:'rf-cancel'}, '*')">Cancel</button>
						...
						const url = `${REMOTE_URL}/pair?room=${state.roomId}&token=${state.pairingToken}&pub=${encodeURIComponent(pubB64)}&relay=${encodeURIComponent(__RELAY_ADDRESS__)}`;
						...
						state.devices
							.map((d) => {
								...
								const statusDot = isActive
									? '<span class="text-success text-small" style="margin-right:8px;" title="Connected">●</span>'
									: '<span class="text-dim text-small" style="margin-right:8px;" title="Offline">○</span>';

								return `
            <div class="list-row modal-row">
              <div style="display: flex; align-items: center;">
                ${statusDot}
                <span class="text-gold" style="font-weight: bold;">${utils.escape(d.name)}</span>
              </div>
              <button class="btn-danger btn-sm" onclick="window.postMessage({type:'rf-del', id:'${d.id}'}, '*')">×</button>
            </div>`;
							})
							.join("") ||
						'<div class="text-center text-dim" style="padding: 20px;">No paired devices.</div>';
			...
			modalInterval = window.setInterval(() => void updateModal(), 5000);
