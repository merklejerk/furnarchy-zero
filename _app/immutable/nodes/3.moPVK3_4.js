import{f as B,e as N,b as T,s as V}from"../chunks/B3FnF0oJ.js";import{i as Qe}from"../chunks/DumpmRuy.js";import{o as Ye}from"../chunks/iqYowYmk.js";import{b3 as gt,aV as vt,af as Ve,as as Ce,at as Ke,ah as Je,ad as we,i as e,R as P,ai as w,m as z,b4 as mt,aj as y,ak as b,ag as et,av as Be,u as W,t as K,y as pt,au as bt}from"../chunks/B34q5ZhI.js";import{i as Q,s as wt,a as yt}from"../chunks/B-8yQNBF.js";import{h as _t}from"../chunks/wBzAbtsP.js";import{s as kt}from"../chunks/DPoAyR-t.js";import{b as zt}from"../chunks/CsEh3Pnl.js";import{M as xt,p as De,f as St,g as Ct,s as ue,d as Lt,h as Mt,c as Pt,m as Et,v as Wt,e as ye,a as Ft}from"../chunks/BCfviV1m.js";import{f as oe}from"../chunks/ByX5xESm.js";import{e as Ut,i as It}from"../chunks/T5S5BKvr.js";import{r as _e,s as Ne,a as ke,c as ze,b as qe,d as Ot}from"../chunks/B8CywloG.js";import{p as xe}from"../chunks/C-xiGS0O.js";import"../chunks/8UtF8RsF.js";function Xe(a,d){gt(window,["resize"],()=>vt(()=>d(window[a])))}function Rt(a,d){let o=d;o.endsWith("/")||(o+="/");const v=a.startsWith("/")?a.substring(1):a;return new URL(v,o).toString()}function At(a,d){const o=new URL(a),v=new URL(d),h=o.pathname.startsWith("/")?o.pathname.substring(1):o.pathname;let R=v.pathname;return R.endsWith("/")||(R+="/"),v.pathname=R+h,v.search=o.search,v.hash=o.hash,v.toString()}function $t(a){return!a.match(/^https?:\/\//)&&!a.startsWith("data:")&&!a.startsWith("blob:")}function Ht(a){return a.startsWith("http://local-server.furcadia.com:8080/")||a.startsWith("https://terra.furcadia.com/")}function Tt(a,d,o){const v=a,h=v.XMLHttpRequest;v.FurcXMLHttpRequest=class extends h{static UNSENT=0;static OPENED=1;static HEADERS_RECEIVED=2;static LOADING=3;static DONE=4;open(S,_,...C){let f=_.toString();return $t(f)?(f=Rt(f,d),console.log(`[FurcXMLHttpRequest] Remapped relative URL to: ${f}`)):Ht(f)&&(f=At(f,o),console.log(`[FurcXMLHttpRequest] Remapped backend URL to: ${f}`)),super.open(S,f,...C)}}}class Ge{buffer="";append(d){this.buffer+=d;const o=[];let v;for(;(v=this.buffer.indexOf(`
`))!==-1;){const h=this.buffer.substring(0,v);o.push(h),this.buffer=this.buffer.substring(v+1)}return o}get isEmpty(){return this.buffer.length===0}clear(){this.buffer=""}}function Bt(a){const d=a,o=d.MessageEvent,v=d.Blob,h=d.Uint8Array,R=d.ArrayBuffer,S={encode:c=>{const t=c.length,n=new h(t);for(let g=0;g<t;g++)n[g]=c.charCodeAt(g)&255;return n}},_={decode:c=>{if(typeof c=="string")return c;let t;c instanceof h?t=c:c instanceof R||c instanceof ArrayBuffer?t=new h(c):t=new h(c.buffer,c.byteOffset,c.byteLength);let n="";const g=t.length,m=32768;for(let k=0;k<g;k+=m){const L=t.subarray(k,Math.min(k+m,g));n+=String.fromCharCode.apply(null,L)}return n}};let C=null,f;d.waitForFurc=new Promise(c=>{f=c});function M(c,t,n,g){t.endsWith(`
`)||(t+=`
`);const m=S.encode(t),k=new o("message",{data:m,origin:"wss://lightbringer.furcadia.com"});k.tag=g,k.sourceId=n,c.dispatchEvent(k)}function F(c,t,n,g){if(c.readyState===WebSocket.OPEN){t.endsWith(`
`)||(t+=`
`);const m=S.encode(t);c.sendTagged?c.sendTagged(m,n,g):c.send(m)}}const x=d.WebSocket;d.FurcWebSocket=class extends x{static CONNECTING=0;static OPEN=1;static CLOSING=2;static CLOSED=3;_outgoingQueue=Promise.resolve();_incomingQueue=Promise.resolve();_incomingBuffer=new Ge;_outgoingBuffer=new Ge;constructor(t,n){if(super(t,n),t.toString().includes("furcadia")||t.toString().includes("6502")){console.log("%c😈 Game Socket Captured","color: #ff00ff;"),C=this;const g=oe;g&&(g.send=(m,k,L)=>F(this,m,k,L),g.inject=(m,k,L)=>M(this,m,k,L),console.log("[Furnarchy] Connected to Game Socket")),this.addEventListener("open",()=>{f(this);const m=oe;m&&m.notifyConnected()}),this.addEventListener("close",()=>{const m=oe;m&&m.notifyDisconnected()})}}send(t){this.sendTagged(t,void 0,void 0)}sendTagged(t,n,g){if(C!==this){super.send(t);return}this._outgoingQueue=this._outgoingQueue.then(async()=>{let m=typeof t=="string"?t:_.decode(t);const k=this._outgoingBuffer.append(m);if(k.length===0)return;const L=[];for(let q of k){const D=oe;if(D){const I=await D.processOutgoing(q,n,g);if(I==null){console.log("%c🚫 Outgoing Dropped (Furnarchy)","color: gray; font-size: 9px");continue}q=I}L.push(q)}if(L.length===0)return;const j=L.join(`
`)+`
`,Z=S.encode(j);super.send(Z)}).catch(m=>{console.error("Error in outgoing message queue:",m)})}_hookMessageEvent(t,n){const g=t.tag,m=t.sourceId;this._incomingQueue=this._incomingQueue.then(async()=>{let k=typeof t.data=="string"?t.data:_.decode(t.data);const L=this._incomingBuffer.append(k);if(L.length===0)return;const j=[];for(let I of L){const Y=oe;if(Y){const ee=await Y.processIncoming(I,m,g);if(ee==null)continue;I=ee}j.push(I)}if(j.length===0)return;const Z=j.join(`
`)+`
`;let q=Z;this.binaryType==="arraybuffer"?q=S.encode(Z).buffer:this.binaryType==="blob"&&(q=new v([S.encode(Z)]));const D=new o("message",{data:q,origin:t.origin,source:t.source});n(D)}).catch(k=>{console.error("Error in incoming message queue:",k)})}set onmessage(t){C===this&&t?super.onmessage=n=>{this._hookMessageEvent(n,g=>{t.call(this,g)})}:super.onmessage=t}get onmessage(){return super.onmessage}addEventListener(t,n,g){if(t==="message"&&C===this&&typeof n=="function"){const m=k=>{this._hookMessageEvent(k,L=>{n.call(this,L)})};super.addEventListener(t,m,g);return}super.addEventListener(t,n,g)}}}async function Dt(a,d){const o=a,v=await fetch(d);if(!v.ok)throw new Error(`Failed to fetch client script: ${v.statusText}`);let h=await v.text();h=h.replaceAll("XMLHttpRequest","FurcXMLHttpRequest"),h=h.replaceAll("WebSocket","FurcWebSocket");const R=h.length;h=(()=>{let _=h.indexOf("Missing login data");if(_===-1)return h;const C=h.substring(0,_),f="__instantiate",M=`
            let ${f} = (cls, ...args) => {
        ...
        `,F=h.substring(_).replace(/;\s*(\w+)\s*=\s*(\w+)\s*\(\s*(\w+)\s*,/,`;${M}; $1 = ${f}($3,`);return C+F})(),R===h.length?console.warn("[Furc Loader] Warning: Could not find game instance assignment point to hook into."):o.processGameClientInstance=_=>{console.log("[Furc Loader] Processing game client instance...");const C=Se(_),f=(()=>{for(const[F,x]of C)if(Ze(x).search(/this(\.\w+){3}\("Reconnecting..."\)/)!==-1)return x.bind(_)})(),M=(()=>{const F=(()=>{for(const[c,t]of C)if(je(t).includes('"Señor Furreton"'))return t})();if(!F)return;const x=(()=>{for(const[c,t]of Se(F))if(je(t).includes('"chatBuffer"'))return t})();if(x){for(const[c,t]of Se(x))if(Ze(t).includes('"specitag"'))return t.bind(x)}})();f||M?(o.__CLIENT_HOOKS={reconnect:f,appendChat:M},console.log("[Furc Loader] Installed client hooks:",o.__CLIENT_HOOKS)):console.warn("[Furc Loader] Warning: Could not establish any client hooks.")};const S=a.document.createElement("script");S.textContent=h,S.async=!0,a.document.body.appendChild(S)}function Se(a){const d=new Set,o=v=>{!v||typeof v!="object"||(Object.getOwnPropertyNames(v).filter(h=>h!=="constructor"&&v[h]).forEach(h=>d.add([h,v[h]])),o(Object.getPrototypeOf(v)))};return o(a),[...d].sort()}function Nt(a){return a&&typeof a=="object"?Object.getPrototypeOf(a).constructor:null}function je(a,d=""){const o=Nt(a);return o?o.toString():d}function Ze(a,d=""){return typeof a=="function"?a.toString():d}var qt=B('<div class="config-btn svelte-8mozrr" title="Configure Plugin">⚙️</div>'),Xt=B('<div class="detail-row svelte-8mozrr"><span class="label svelte-8mozrr">ID:</span> </div>'),Gt=B('<div class="detail-row svelte-8mozrr"><span class="label svelte-8mozrr">Version:</span> </div>'),jt=B('<div class="detail-row svelte-8mozrr"><span class="label svelte-8mozrr">Author:</span> </div>'),Zt=B('<div class="detail-row svelte-8mozrr"><span class="label svelte-8mozrr">Description:</span> </div>'),Qt=B('<div class="plugin-details svelte-8mozrr"><!> <!> <!> <!> <div class="detail-row svelte-8mozrr"><span class="label svelte-8mozrr">URL:</span> <a target="_blank" rel="noopener noreferrer" class="url-link svelte-8mozrr"> </a></div> <div class="button-group svelte-8mozrr"><button class="action-btn reload-btn svelte-8mozrr">Reload Plugin</button> <button class="action-btn remove-btn svelte-8mozrr">Remove Plugin</button></div></div>'),Yt=B('<li><div class="plugin-header svelte-8mozrr"><div class="plugin-title-group svelte-8mozrr"><div></div> <!> <span class="plugin-name svelte-8mozrr"> </span></div></div> <!></li>'),Vt=B('<li class="empty svelte-8mozrr">No plugins installed</li>'),Kt=B('<div class="modal-backdrop svelte-8mozrr"></div> <div class="modal retro-theme svelte-8mozrr"><div class="header-row svelte-8mozrr"><h2 class="svelte-8mozrr"></h2> <button class="close-btn svelte-8mozrr" aria-label="Close">✕</button></div> <div class="section-title svelte-8mozrr">Display Settings</div> <div class="display-settings svelte-8mozrr"><div class="setting-row svelte-8mozrr"><label for="zoom-control" class="svelte-8mozrr"> </label> <input id="zoom-control" type="range" min="0.5" max="3.0" step="0.1" class="svelte-8mozrr"/></div> <div class="setting-row svelte-8mozrr"><label class="checkbox-label svelte-8mozrr"><input type="checkbox" class="svelte-8mozrr"/> Fit to Window Width</label></div></div> <div class="section-title svelte-8mozrr">Plugins</div> <div class="add-plugin svelte-8mozrr"><input type="text" placeholder="https://example.com/my-plugin.js" class="svelte-8mozrr"/> <button class="svelte-8mozrr"> </button></div> <ul class="plugin-list svelte-8mozrr"><!> <!></ul> <div class="footer svelte-8mozrr"><small>Plugins have full access to your game session.</small></div></div>',1),Jt=B('<div class="plugin-manager svelte-8mozrr"><button class="fab svelte-8mozrr" title="Furnarchy Menu"><!></button> <!></div>');function ei(a,d){Ve(d,!1);const o=()=>yt(De,"$pluginStore",v),[v,h]=wt(),R=P();let S=P(!1),_=P(""),C=P(null),f=P(!1),M=P({}),F=P(!1),x=xe(d,"zoomLevel",12,1.5),c=xe(d,"fitWidth",12,!1),t=xe(d,"isMobileMode",8,!1),n=oe;Ye(()=>{const i=St();x(i.zoomLevel),c(i.fitWidth),z(F,!0),window.Furnarchy=n.getExposedAPI(),De.set(Ct()),n.plugins&&n.plugins.forEach(l=>{l.metadata.sourceUrl&&mt(M,e(M)[l.metadata.sourceUrl]=l._handlers.configure.length>0)}),n.onRegister(l=>{const r=l.metadata||l,X=r.sourceUrl||l.sourceUrl;if(X){z(M,{...e(M),[X]:l._handlers.configure.length>0});const J=o().findIndex(re=>re.url===X);if(J!==-1){const re=n.loadingPluginUrl===X,O=o()[J];let $=!1;const H={...O};if(re){const se=r.toggle?!1:O.enabled!==!1;l._setEnabled(se),r.toggle&&O.enabled!==!1&&(H.enabled=!1,$=!0)}else l.enabled!==(O.enabled!==!1)&&(H.enabled=l.enabled,$=!0);if(O.name!==r.name&&(H.name=r.name,$=!0),O.id!==r.id&&(H.id=r.id,$=!0),O.description!==r.description&&(H.description=r.description,$=!0),O.version!==r.version&&(H.version=r.version,$=!0),O.author!==r.author&&(H.author=r.author,$=!0),O.toggle!==r.toggle&&(H.toggle=r.toggle,$=!0),$){const se=[...o()];se[J]=H,ue(se)}}}});const u=n.version||"0.0.0";Lt(u).then(()=>{q()})});function g(){z(S,!e(S))}async function m(){if(e(_)&&!o().some(i=>i.url===e(_))){z(f,!0);try{const i=await Wt(e(_)),u=[...o(),{id:i.id,url:e(_),name:i.name,description:i.description,version:i.version,author:i.author,enabled:i.toggle!==void 0?!i.toggle:!0,toggle:i.toggle}];ue(u),D(e(_)),z(_,"")}catch(i){alert(`Failed to verify plugin: ${i.message||i}`)}finally{z(f,!1)}}}function k(i){const u=o().find(r=>r.url===i);u&&u.id&&Et(u.id);const l=o().filter(r=>r.url!==i);ue(l),confirm("Plugin removed. Reload page to take effect?")&&window.location.reload()}function L(i){const u=o().map(l=>{if(l.url===i){const r=l.enabled===void 0?!1:!l.enabled;if(n&&n.plugins){const X=n.plugins.find(J=>J.metadata.sourceUrl===i);X&&X._setEnabled(r)}return{...l,enabled:r}}return l});ue(u)}function j(i){if(z(S,!1),n&&n.plugins){const u=n.plugins.find(l=>l.metadata.sourceUrl===i);u&&u._notifyConfigure()}}async function Z(i){const u=document.querySelector(`script[data-plugin-url="${i}"]`);if(u&&u.remove(),n&&n.plugins){const l=n.plugins.findIndex(r=>r.metadata.sourceUrl===i);l!==-1&&(n.plugins[l]._setEnabled(!1),n.plugins.splice(l,1))}await D(i)}async function q(){await Promise.all(o().map(i=>D(i.url))),n.start()}async function D(i){if(!document.querySelector(`script[data-plugin-url="${i}"]`))try{const u=await fetch(i);if(!u.ok)throw new Error(`HTTP ${u.status}`);const l=await u.text();n.loadingPluginUrl=i;const r=document.createElement("script");r.textContent=l,r.dataset.pluginUrl=i,document.body.appendChild(r),console.log(`[PluginManager] Loaded via fetch: ${i}`)}catch(u){return console.warn(`[PluginManager] Fetch failed for ${i}, falling back to script tag.`,u),new Promise(l=>{const r=document.createElement("script");r.src=i,r.async=!0,r.dataset.pluginUrl=i,r.onload=()=>{console.log(`[PluginManager] Loaded via tag: ${i}`),l()},r.onerror=()=>{console.error(`[PluginManager] Failed to load: ${i}`),l()},document.body.appendChild(r)})}finally{n.loadingPluginUrl=null}}Ce(()=>o(),()=>{z(R,[...o()].sort((i,u)=>{const l=i.enabled!==!1,r=u.enabled!==!1;return l!==r?l?-1:1:(i.name||i.url).localeCompare(u.name||u.url)}))}),Ce(()=>(e(F),we(x()),we(c())),()=>{typeof window<"u"&&e(F)&&Mt({zoomLevel:x(),fitWidth:c()})}),Ke(),Qe();var I=Jt(),Y=w(I),ee=w(Y);xt(ee),y(Y);var p=b(Y,2);{var A=i=>{var u=Kt(),l=et(u),r=b(l,2),X=w(r),J=w(X);J.textContent="Furnarchy Zero 0.19.0";var re=b(J,2);y(X);var O=b(X,4),$=w(O),H=w($),se=w(H);y(H);var he=b(H,2);_e(he),y($);var Le=b($,2),Me=w(Le),fe=w(Me);_e(fe),Be(),y(Me),y(Le),y(O);var ge=b(O,4),ae=w(ge);_e(ae);var ce=b(ae,2),tt=w(ce,!0);y(ce),y(ge);var Pe=b(ge,2),Ee=w(Pe);Ut(Ee,1,()=>e(R),It,(G,s)=>{var le=Yt();let We;var ve=w(le),Fe=w(ve),de=w(Fe);let Ue;var Ie=b(de,2);{var ot=te=>{var ie=qt();N("click",ie,ze(()=>j(e(s).url))),T(te,ie)};Q(Ie,te=>{e(M),e(s),W(()=>e(M)[e(s).url])&&te(ot)})}var me=b(Ie,2),st=w(me,!0);y(me),y(Fe),y(ve);var rt=b(ve,2);{var at=te=>{var ie=Qt(),Oe=w(ie);{var lt=E=>{var U=Xt(),ne=b(w(U));y(U),K(()=>V(ne,` ${e(s),W(()=>e(s).id)??""}`)),T(E,U)};Q(Oe,E=>{e(s),W(()=>e(s).id)&&E(lt)})}var Re=b(Oe,2);{var ct=E=>{var U=Gt(),ne=b(w(U));y(U),K(()=>V(ne,` ${e(s),W(()=>e(s).version)??""}`)),T(E,U)};Q(Re,E=>{e(s),W(()=>e(s).version)&&E(ct)})}var Ae=b(Re,2);{var dt=E=>{var U=jt(),ne=b(w(U));y(U),K(()=>V(ne,` ${e(s),W(()=>e(s).author)??""}`)),T(E,U)};Q(Ae,E=>{e(s),W(()=>e(s).author)&&E(dt)})}var $e=b(Ae,2);{var ut=E=>{var U=Zt(),ne=b(w(U));y(U),K(()=>V(ne,` ${e(s),W(()=>e(s).description)??""}`)),T(E,U)};Q($e,E=>{e(s),W(()=>e(s).description)&&E(ut)})}var pe=b($e,2),be=b(w(pe),2),ht=w(be,!0);y(be),y(pe);var He=b(pe,2),Te=w(He),ft=b(Te,2);y(He),y(ie),K(()=>{ke(be,"href",(e(s),W(()=>e(s).url))),V(ht,(e(s),W(()=>e(s).url)))}),N("click",Te,()=>Z(e(s).url)),N("click",ft,()=>k(e(s).url)),N("click",ie,ze(function(E){Pt.call(this,d,E)})),T(te,ie)};Q(rt,te=>{e(C),e(s),W(()=>e(C)===e(s).url)&&te(at)})}y(le),K(()=>{We=Ne(le,1,"plugin-item svelte-8mozrr",null,We,{expanded:e(C)===e(s).url,disabled:e(s).enabled===!1&&!e(s).toggle}),Ue=Ne(de,1,"toggle-switch svelte-8mozrr",null,Ue,{checked:e(s).enabled!==!1}),ke(de,"title",(e(s),W(()=>e(s).enabled!==!1?"Disable Plugin":"Enable Plugin"))),ke(me,"title",(e(s),W(()=>e(s).url))),V(st,(e(s),W(()=>e(s).name||e(s).url)))}),N("click",de,ze(()=>L(e(s).url))),N("click",le,()=>z(C,e(C)===e(s).url?null:e(s).url)),T(G,le)});var it=b(Ee,2);{var nt=G=>{var s=Vt();T(G,s)};Q(it,G=>{o(),W(()=>o().length===0)&&G(nt)})}y(Pe),Be(2),y(r),K(G=>{V(se,`Zoom: ${G??""}x`),he.disabled=c()||t(),fe.disabled=t(),ae.disabled=e(f),ce.disabled=e(f),V(tt,e(f)?"...":"Add")},[()=>(we(x()),W(()=>x().toFixed(1)))]),N("click",l,g),N("click",re,g),qe(he,x),Ot(fe,c),qe(ae,()=>e(_),G=>z(_,G)),N("keydown",ae,G=>G.key==="Enter"&&!e(f)&&m()),N("click",ce,m),T(i,u)};Q(p,i=>{e(S)&&i(A)})}y(I),N("click",Y,g),T(a,I),Je(),h()}const ti=`
<!DOCTYPE html>
<html>
<head>
...
`;var ii=B('<div class="loading svelte-hy9bcf"><p>Loading Furcadia Web Client...</p></div>'),ni=B('<div class="error svelte-hy9bcf"><p> </p> <button class="svelte-hy9bcf">Try Again</button></div>'),oi=B('<!> <!> <div class="iframe-container svelte-hy9bcf"><iframe title="Furcadia Client" class="game-iframe svelte-hy9bcf"></iframe></div> <!>',1);function _i(a,d){Ve(d,!1);const o=P(),v=ye.PUBLIC_FURCADIA_CLIENT_JS_URL,h=ye.PUBLIC_AUTH_PROXY_URL,R=ye.PUBLIC_PLAY_FURCADIA_URL;let S=P(!1),_=P(""),C=!1,f=P(),M=P(640),F=P(480),x=P(!1),c=P(1.5),t=P(!1),n=P(1024),g=P(768);Ye(()=>{const A=Ft()||h;if(console.log(`[Furnarchy] Using backend URL: ${A}`),window.addEventListener("message",m),e(f)&&e(f).contentWindow&&e(f).contentDocument){const i=e(f).contentDocument,u=e(f).contentWindow;i.open(),i.write(ti),i.close(),Tt(u,R,A),Bt(u),oe.attachInputInterception(i),k(u)}return()=>{window.removeEventListener("message",m)}});function m(p){p.data&&p.data.type==="resize"&&(z(M,Math.ceil(p.data.width)),z(F,Math.ceil(p.data.height)),z(x,!!p.data.isMobile))}async function k(p){if(!C){z(S,!0),z(_,"");try{await Dt(p,v),C=!0}catch(A){console.error(A),z(_,A.message||"An unknown error occurred")}finally{z(S,!1)}}}Ce(()=>(e(x),e(t),e(n),e(M),e(g),e(F),e(c)),()=>{z(o,e(x)?1:e(t)?Math.min(e(n)/e(M),e(g)/e(F)):e(c))}),Ke(),Qe();var L=oi();_t("hy9bcf",p=>{pt(()=>{bt.title="Furnarchy Zero"})});var j=et(L);ei(j,{get isMobileMode(){return e(x)},get zoomLevel(){return e(c)},set zoomLevel(p){z(c,p)},get fitWidth(){return e(t)},set fitWidth(p){z(t,p)},$$legacy:!0});var Z=b(j,2);{var q=p=>{var A=ii();T(p,A)};Q(Z,p=>{e(S)&&p(q)})}var D=b(Z,2),I=w(D);zt(I,p=>z(f,p),()=>e(f)),y(D);var Y=b(D,2);{var ee=p=>{var A=ni(),i=w(A),u=w(i);y(i);var l=b(i,2);y(A),K(()=>V(u,`Error: ${e(_)??""}`)),N("click",l,()=>window.location.reload()),T(p,A)};Q(Y,p=>{e(_)&&p(ee)})}K(()=>kt(I,`width: ${e(x)?"100%":e(M)+"px"}; height: ${e(x)?"100%":e(F)+"px"}; --zoom: ${e(o)??""};`)),Xe("innerWidth",p=>z(n,p)),Xe("innerHeight",p=>z(g,p)),T(a,L),Je()}export{_i as component};
