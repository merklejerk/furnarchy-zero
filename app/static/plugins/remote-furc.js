(function() {
  ...
  async function encrypt(data, key, hint) {
  ...
  async function deriveSAS(sharedKey, hostPub, remotePub) {
  ...
  async function loadSettings(api, state) {
    ...
    if (settings.devices) {
      for (const d of settings.devices) {
        ...
        state.devices.push({
          ...
          lastSeen: 0
          // Reset activity on load
        });
      }
    }
  }
  async function saveSettings(api, state) {
    const key = state.currentUser ? `settings_${state.currentUser.uid}` : null;
    if (!key) return;
  ...
  Furnarchy.register(
    ...
    (api) => {
      ...
      const CONFIG = {
        HEARTBEAT_MS: 15e3,
        RECONNECT_MS: 5e3,
        INDICATOR_REFRESH_MS: 3e4,
        ACTIVE_DEVICE_TIMEOUT_MS: 12e4,
        // For broadcasting
      ...
      async function broadcastToActive(msg, timeoutMs = CONFIG.ACTIVE_DEVICE_TIMEOUT_MS) {
        const now = Date.now();
      ...
      function updateIndicator() {
        ...
        if (!el) {
          ...
          el.style.background = "rgba(80, 85, 139, 0.9)";
        ...
        const label = activeDevices.length === 1 ? activeDevices[0].name : `${activeDevices.length} Remotes`;
      ...
      async function handleHandshake(json) {
        try {
          ...
          if (msg.type === "HANDSHAKE_INIT" && state.isPairingMode && state.pairingToken && msg.token === state.pairingToken) {
            state.pairingToken = "";
      ...
      async function finalizePairing(deviceName) {
        var _a;
        if (!state.pendingPairing || !state.pendingPairing.isVerified || !state.pendingPairing.sharedKey)
          return;
        ...
        try {
          ...
          void sendToDevice(device, {
            type: "HANDSHAKE_ACK",
            name: ((_a = api.gameState.player) == null ? void 0 : _a.name) || "Unknown Host"
          });
