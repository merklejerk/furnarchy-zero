...
export function installWebSocketPatch() {
    ...
    (window as any).FurcWebSocket = class FurcWebSocket extends WebSocket {
        ...
        constructor(url: string | URL, protocols?: string | string[]) {
            ...
            if (url.toString().includes('furcadia') || url.toString().includes('6502')) {
                ...
                if (furnarchy) {
                    ...
                    furnarchy.inject = (text: string, tag: string = "PLUGIN") => injectIntoSocket(this, text, tag);
        ...
        sendTagged(data: string | ArrayBufferLike | Blob | ArrayBufferView, tag?: string): void {
            ...
            this._outgoingQueue = this._outgoingQueue.then(async () => {
        ...
        private _hookMessageEvent(msgEvent: MessageEvent, callback: (evt: MessageEvent) => void) {
            ...
            this._incomingQueue = this._incomingQueue.then(async () => {
                // Decode the message
                let text = typeof msgEvent.data === 'string' ? msgEvent.data : decoder.decode(msgEvent.data);
