...
export interface FoxSpriteEntry {
	...
	height: number;
	hasAlpha: boolean;
	// Decoded pixel data for this sprite (RGBA or indexed bytes)
	data?: Uint8Array;
	// Compressed LZMA stream (stored for lazy hydration)
	compressedData?: Uint8Array;
}

export interface FoxSpriteLayer {
	purpose?: SpritePurpose; // C
	imageID?: number; // c (Raw value from tag)
	spriteIndex?: number; // Resolved 0-based index (c - 1)
	offset?: { x: number; y: number }; // O
}

export interface FoxFrame {
	frameOffset?: { x: number; y: number }; // o
	furreOffset?: { x: number; y: number }; // f
	sprites: FoxSpriteLayer[];
}

export interface FoxShape {
	purpose?: ShapePurpose; // p
	state?: number; // s (Raw bitfield)
	direction?: FoxDirection; // D
	ratio?: { n: number; d: number }; // R
	kitterSpeak?: unknown[]; // K
	frames: FoxFrame[];
}
...
export interface FoxObject {
	...
	moreFlags?: number; // ?
	editType?: EditType; // t
	license?: LicenseType; // l
	fxFilter?: { layer: number; mode: number }; // F
	shapes: FoxShape[];
}
 ...
 */
export function xorFox(data: Uint8Array, seed: Uint8Array, modifier: number): Uint8Array {
...
const LEGACY_DEFAULT_LAST_ID_START: Record<number, number> = {
...
const MODERN_DEFAULT_LAST_ID_START: Record<number, number> = {
 ...
 */
export async function decompressFoxSprite(sprite: FoxSpriteEntry): Promise<Uint8Array> {
 ...
 */
export async function parseFox5(buffer: Uint8Array, options?: ParseFoxOptions): Promise<FoxFile> {
	...
	const file: FoxFileLayer = { objects: [] };
	...
	while (ctx.pos < ctx.u8.length) {
		...
		if (tag === 76) {
			// 'L'
			ctx.pos++;
			const level = ctx.u8[ctx.pos++] as FoxLevel;
	...
	for (const s of ctx.spriteTable) {
		...
		const spriteEntry: FoxSpriteEntry = {
			id: s.id,
			offset: s.offset,
			size: s.size,
			width: s.width,
...
function readString(ctx: FoxContext): string {
...
function readFileBlock(ctx: FoxContext, file: FoxFileLayer) {
	while (ctx.pos < ctx.u8.length) {
		const tag = ctx.u8[ctx.pos++];
...
function readObjectBlock(ctx: FoxContext, obj: FoxObject) {
	while (ctx.pos < ctx.u8.length) {
		const tag = ctx.u8[ctx.pos++];
		if (tag === 60) return; // '<'
		...
		switch (tag) {
			...
			case 107: // 'k' Keywords
				...
				break;
			case 108: // 'l' License
				obj.license = ctx.u8[ctx.pos++] as LicenseType;
				break;
			case 33: // '!' Flags
			...
			case 116: // 't' Edit Type
				obj.editType = ctx.u8[ctx.pos++] as EditType;
				break;
			case 70: // 'F' FX Filter
...
function readShapeBlock(ctx: FoxContext, shape: FoxShape) {
	while (ctx.pos < ctx.u8.length) {
		const tag = ctx.u8[ctx.pos++];
		if (tag === 60) return; // '<'
		...
		switch (tag) {
			case 0:
				break;
			case 112: // 'p' Purpose
				shape.purpose = ctx.u8[ctx.pos++] as ShapePurpose;
				break;
			case 115: // 's' State
				shape.state = ctx.u8[ctx.pos++];
				break;
			case 68: // 'D' Direction
				shape.direction = ctx.u8[ctx.pos++] as FoxDirection;
				break;
			case 82: // 'R' Ratio
...
function readFrameBlock(ctx: FoxContext, frame: FoxFrame) {
	while (ctx.pos < ctx.u8.length) {
		const tag = ctx.u8[ctx.pos++];
		if (tag === 60) return; // '<'
...
function readSpriteLayerBlock(ctx: FoxContext, layer: FoxSpriteLayer) {
	while (ctx.pos < ctx.u8.length) {
		const tag = ctx.u8[ctx.pos++];
		if (tag === 60) return; // '<'
		...
		switch (tag) {
			case 0:
				break;
			case 67: // 'C' Purpose
				layer.purpose = ctx.view.getUint16(ctx.pos) as SpritePurpose;
				ctx.pos += 2;
				break;
			case 99: // 'c' Image ID
