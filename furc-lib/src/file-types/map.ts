 ...
 */
function decryptMapBody(buffer: Uint8Array, useTableB = false): Uint8Array {
 ...
 */
export function parseMap(buffer: Uint8Array): MapData {
	...
	const readLayer = (name: string, isUint8 = false, step = 1, useBE = false) => {
		const layer = isUint8
	...
	// 1. Floors
	mapData.rawFloors = readLayer('Floors', false, 1, isModernEncrypted) as Uint16Array;
	...
	// 2. Objects
	mapData.rawObjects = readLayer('Objects', false, 1, isModernEncrypted) as Uint16Array;
	...
	// 3. Walls
	const rawWalls = readLayer('Walls', true, 2) as Uint8Array;
	...
	if (meta.version > 130) {
		mapData.regions = readLayer('Regions', false, 1, isModernEncrypted) as Uint16Array;
	}

	// Layer 5: Effects (> 1.30)
	// In the client, this is the 'al' layer used for effects/animations.
	if (meta.version > 130) {
		mapData.rawEffects = readLayer('Effects', false, 1, isModernEncrypted) as Uint16Array;
	...
	if (meta.version >= 150) {
		mapData.lighting = readLayer('Lighting', false, 1, isModernEncrypted) as Uint16Array;
	}

	// Layer 7: Ambiance (>= 1.50)
	if (meta.version >= 150) {
		mapData.ambiance = readLayer('Ambiance', false, 1, isModernEncrypted) as Uint16Array;
	}
