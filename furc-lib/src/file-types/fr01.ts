...
export class Fr01Archive {
	private files: Map<string, Fr01FileEntry> = new Map();

	constructor(buffer: Uint8Array) {
		this.parse(buffer);
	}

	private parse(buffer: Uint8Array) {
	...
	public entries(): IterableIterator<[string, Fr01FileEntry]> {
		return this.files.entries();
	}

	public async getFile(filename: string): Promise<Uint8Array> {
	...
	private async decompress(entry: Fr01FileEntry): Promise<Uint8Array> {
